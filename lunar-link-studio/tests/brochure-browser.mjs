...
import {MODEL_VERSION} from '../src/engine.js';
...
try{
  ...
  for(const name of ['Power input / DC-DC','Motor drivers / current sense','MCU / attitude / encoder interface','Host-powered payload heater pad','Parasitic thermal path to lander interface','Lander power / data input'])assert.ok(full.nodes.some(n=>n.name===name),name);
  ...
  assert.match(await page.locator('#design-fit-summary').textContent(),/SAMPLED FIT ONLY/);
  ...
  assert.equal(study.modelVersion,MODEL_VERSION);assert.equal(study.assembly.allSampledFit,true);assert.equal(study.electrical.outagePass,false);
