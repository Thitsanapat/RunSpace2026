...
// Visible features reconstructed from two brochure crops, not provider CAD.
...
export function buildSurfaceLander(view) {
  ...
  const silver=material('#c9ccca',0.45,0.48),ivory=material('#e5e5dc'),blanket=material('#686b67',0.25,0.86),seam=material('#454944');
  const red=material('#ef2439'),blue=material('#345de5'),yellow=material('#dedb29'),green=material('#70e138'),black=material('#303634');
  ...
  for(let i=0;i<outline.length;i++){
    ...
    const wall=new THREE.Group();wall.position.set((a[0]+b[0])/2,0.30,(a[1]+b[1])/2);wall.rotation.y=-Math.atan2(dz,dx);lander.add(wall);
  ...
  // Raised grey equipment towers and six recessed circular ports on each face.
  function tower(sx,sz){
    const group=new THREE.Group();group.position.set(sx*0.85,top+0.27,sz*0.68);group.rotation.y=-sx*sz*Math.PI/4;lander.add(group);
    block([0.39,0.55,0.18],blanket,group,[0,0,0]);box([0.36,0.53,0.018],silver,group,[0,0,0.10]);
    for(const xx of [-0.087,0.087])for(const yy of [-0.17,0,0.17]){
      const hole=mesh(new THREE.CylinderGeometry(0.053,0.053,0.021,24),black,group,[xx,yy,0.114]);hole.rotation.x=Math.PI/2;
      mesh(new THREE.TorusGeometry(0.055,0.007,8,24),silver,group,[xx,yy,0.129]);
      const disc=mesh(new THREE.CircleGeometry(0.042,24),ivory,group,[xx+0.004,yy-0.004,0.13]);disc.scale.y=0.65;
    }
    for(const yy of [-0.28,0.28])rod([-0.20,yy,0.13],[0.20,yy,0.13],0.012,silver,group);
    for(const xx of [-0.2,0.2])rod([xx,-0.28,0.13],[xx,0.28,0.13],0.012,silver,group);
    const flap=block([0.3,0.53,0.025],blanket,group,[-0.32,0,0.04]);flap.rotation.y=-0.28;
  }
  tower(1,-1);tower(-1,1);
  for(const sx of [-1,1]){
  ...
  for(const sx of [-1,1])for(const sz of [-1,1]){
    ...
    rod([sx*0.92,0.40,sz*0.75],foot,0.040,silver);rod([sx*0.88,0.35,sz*0.72],knee,0.062,black);
  ...
  torus(0.67,0.023,-0.45);torus(0.62,0.009,-0.485);
