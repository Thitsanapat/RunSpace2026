...
const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}"><rect width="100%" height="100%" fill="#fff"/>${style}<text x="28" y="35" class="title">OUR ANALYSIS · PAPER-COMPARABLE RF EVIDENCE</text><text x="28" y="58" class="sub">S-band 2.205 GHz · ${c.peakGain.toFixed(1)} dBic target · 5 W RF · payload transceiver/PA · lander DC only · reduced-order model</text>${s11}${margin}${polarPanels}<text x="600" y="568" text-anchor="middle" class="tiny">Comparable layout: Gadhafi et al., Sensors 2024, Fig. 18 (2.42 GHz CST). Frequencies and evidence levels differ; direct performance ranking is not valid.</text></svg>`;
