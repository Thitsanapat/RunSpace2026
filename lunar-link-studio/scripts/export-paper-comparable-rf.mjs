...
function polar(cx,cy,R,label,steer){
  ...
  return `${grid}<path d="${curve(0)}" class="fixed" stroke-width="3"/><path d="${curve(steer)}" class="gimbal" stroke-width="3"/><text x="${cx}" y="${cy+R+34}" text-anchor="middle" class="panel">${label}</text><text x="${cx}" y="${cy+R+50}" text-anchor="middle" class="tiny">Realized gain relative to 6.5 dBic peak (dB)</text>`;
}
...
const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}"><rect width="100%" height="100%" fill="#fff"/>${style}<text x="28" y="35" class="title">OUR ANALYSIS · PAPER-COMPARABLE RF EVIDENCE</text><text x="28" y="58" class="sub">S-band 2.205 GHz · 6.5 dBic input · 5 W RF · 384,400 km · 4 kbps · reduced-order model, not CST or measured data</text>${s11}${margin}${polarPanels}<text x="600" y="568" text-anchor="middle" class="tiny">Comparable layout: Gadhafi et al., Sensors 2024, Fig. 18 (2.42 GHz CST). Frequencies and evidence levels differ; direct performance ranking is not valid.</text></svg>`;
