...
const angles=Array.from({length:181},(_,i)=>i-90).map(a=>({a,g:directionalGain(Math.abs(a),0,DEFAULTS),loss:DEFAULTS.peakGain-directionalGain(Math.abs(a),0,DEFAULTS)}));
{
  ...
  for(const g of [-30,-20,-10,0,DEFAULTS.peakGain])grid+=`<line x1="90" y1="${Y(g)}" x2="1130" y2="${Y(g)}" class="grid"/><text x="74" y="${Y(g)+5}" class="small" text-anchor="end">${g}</text>`;
  for(const a of [-90,-60,-beam.halfAngle,-30,0,30,beam.halfAngle,60,90])grid+=`<line x1="${X(a)}" y1="180" x2="${X(a)}" y2="590" class="grid"/><text x="${X(a)}" y="618" class="small" text-anchor="middle">${f(a,a%1?1:0)}°</text>`;
  const half=DEFAULTS.peakGain-3.0103;
  await save('01-gain-vs-off-axis-angle',`<text x="38" y="50" class="title">S-band antenna gain vs off-axis angle</text><text x="38" y="80" class="sub">2.205 GHz · ${f(DEFAULTS.peakGain,1)} dBic target · analytical axisymmetric cosine pattern · not measured mounted data</text><rect x="38" y="110" width="1124" height="540" rx="12" class="card"/><rect x="${X(-beam.halfAngle)}" y="180" width="${X(beam.halfAngle)-X(-beam.halfAngle)}" height="410" fill="#ebfbee" opacity=".65"/>${grid}<line x1="90" y1="${Y(half)}" x2="1130" y2="${Y(half)}" stroke="#66a80f" stroke-width="2" stroke-dasharray="8 6"/><path d="${line(angles,X,Y,'a','g')}" fill="none" stroke="#1971c2" stroke-width="6"/><circle cx="${X(0)}" cy="${Y(DEFAULTS.peakGain)}" r="7" fill="#1971c2"/><circle cx="${X(-beam.halfAngle)}" cy="${Y(half)}" r="7" fill="#66a80f"/><circle cx="${X(beam.halfAngle)}" cy="${Y(half)}" r="7" fill="#66a80f"/><text x="${X(0)-15}" y="${Y(DEFAULTS.peakGain)-15}" class="label blue" text-anchor="end">Peak target ${f(DEFAULTS.peakGain,1)} dBic</text><text x="${X(beam.halfAngle)+12}" y="${Y(half)-12}" class="label green">−3 dB at ±${f(beam.halfAngle,2)}°</text><rect x="695" y="126" width="430" height="76" rx="10" class="warn"/><text x="715" y="157" class="label orange">HPBW ≈ ${f(DEFAULTS.beamwidth,2)}°</text><text x="715" y="184" class="small">±0.5° control error → ≈${f(beam.lossAtHalfDegree,4)} dB loss</text><text x="610" y="686" class="label" text-anchor="middle">Signed off-axis angle from boresight (deg)</text><text x="25" y="410" class="label" text-anchor="middle" transform="rotate(-90 25 410)">Realized gain target model (dBic)</text>`);
}

{
  ...
  const body=`<text x="34" y="39" class="title">OUR ANALYSIS · EARTH–MOON RF EVIDENCE</text><text x="34" y="68" class="sub">S-band 2.205 GHz · 384,400 km · 5 W RF · ${f(DEFAULTS.peakGain,1)} dBic target · 4 kbps · G/T 22 dB/K assumed</text>
  ...
  <rect x="296" y="88" width="250" height="56" rx="9" class="card"/><text x="313" y="111" class="small">EIRP</text><text x="528" y="128" class="h" text-anchor="end">${f(on.eirp)} dBW</text>
  ...
  <text x="876" y="219" class="label orange">FIXED PATCH</text><text x="876" y="260" class="h orange">${f(fixed65)} dB</text><text x="876" y="287" class="small">after reserve · FAIL</text><line x1="876" y1="311" x2="1140" y2="311" class="grid"/>
  <text x="876" y="350" class="label green">GIMBAL ON-AXIS</text><text x="876" y="391" class="h green">+${f(gimbal)} dB</text><text x="876" y="418" class="small">after reserve · PASS*</text><text x="876" y="458" class="small">Recovered headroom: +${f(gimbal-fixed65)} dB</text><text x="876" y="488" class="small">*Clear-path RF budget only</text>
...
const evidence=[
  ['Gain vs angle','AVAILABLE','Analytical cosine; 5.2 dBic target','Replace with mounted chamber pattern'],
  ...
  ['S11 vs frequency','MISSING','Scalar −10 dB requirement','Mounted VNA sweep incl. feed/coax'],
